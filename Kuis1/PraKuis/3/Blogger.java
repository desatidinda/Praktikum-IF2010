public class Blogger implements Poster {
        ...
        if (blog instanceof Blog) {
            ...
            if (blogCount < maxBlogs) {
                ...
                for (int i = 0; i < subscriberCount; i++) {
                    subscribers[i].addNotification("Blog " + blogName + " telah mengunggah blog baru dengan judul " + blog.getTitle());
                }
            }
        }
    }
    ...
    public void changePost(Content oldBlog, Content newBlog) {
        if (blogs != null && oldBlog instanceof Blog && newBlog instanceof Blog) {
            for (int i = 0; i < blogCount; i++) {
                if (blogs[i] != null && blogs[i].isSame(oldBlog)) {
                    blogs[i] = (Blog) newBlog;

                    for (int j = 0; j < subscriberCount; j++) {
                        subscribers[j].addNotification("Blog " + blogName + " telah mengubah blog " + oldBlog.getTitle() + " menjadi " + newBlog.getTitle());
                    }
                    return;
                }
            }
