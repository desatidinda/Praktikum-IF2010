public class Channel implements Poster {
    ...
    public void post(Content video) {
        if (video instanceof Video) {
            ...
            if (videoCount < maxVideos) {
                videos[videoCount] = vid;
                videoCount++;

                for (int i = 0; i < subscriberCount; i++) {
                    subscribers[i].addNotification("Channel " + channelName + " telah mengunggah video baru dengan judul " + video.getTitle());
                }
            }
        }
    }
    ...
    public void changePost(Content oldVideo, Content newVideo) {
        if (videos != null && oldVideo instanceof Video && newVideo instanceof Video) {
            for(int i = 0; i < videoCount; i++) {
                if (videos[i] != null && videos[i].isSame(oldVideo)) {
                    videos[i] = (Video) newVideo;

                    for (int j = 0; j < subscriberCount; j++) {
                        subscribers[j].addNotification("Channel " + channelName + " telah mengubah video " + oldVideo.getTitle() + " menjadi " + newVideo.getTitle());
                    }
                    return;
                }
            }
