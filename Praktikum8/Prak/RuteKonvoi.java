...
public class RuteKonvoi {
    ...
    public void addSegmen(String namaSegment, int estimasiWaktuMenit) {
        if (namaSegment != null && estimasiWaktuMenit > 0) {
            SegmenRute rute = new SegmenRute(namaSegment, estimasiWaktuMenit);
            daftarSegmenRute.add(rute);
        }
    }
    ...
    public int hitungTotalEstimasiWaktu() {
        ...
        for (int i = 0; i < daftarSegmenRute.size(); i++) {
            SegmenRute rute = daftarSegmenRute.get(i);
            total += rute.getEstimasiWaktuSegmentMenit();
        }
        return total;
    }
