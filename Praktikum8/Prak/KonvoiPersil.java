...
public class KonvoiPersil {
    ...
    public Pendukung cariPendukungTertuaDalamKendaraan(Kendaraan kendaraan) {
        if (kendaraan == null || kendaraan.getDaftarPendukung().isEmpty()) {
            ...
            List<Pendukung> bobotoh = kendaraan.getDaftarPendukung();
            int max = 0;
            int index = 0;
            for (int i = 0; i < bobotoh.size(); i++) {
                Pendukung orang = bobotoh.get(i);
                if (orang.getUmur() > max) {
                    max = orang.getUmur();
                    index = i;
                }
            }
            return bobotoh.get(index);
        }
    }
