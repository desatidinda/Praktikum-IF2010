...
public class Mahasiswa {
    ...
    public Mahasiswa(String nim, String nama) throws NimAlreadyExistsException {
        if (registeredNim.contains(nim)) {  //cek nimnya ud terdaftar blm
            throw new NimAlreadyExistsException(); //kl udah throw exception
        }
        this.nim = nim;  //constructor biasa
        this.nama = nama;
        this.nilai = new HashMap<String, String>(); //bikin map nilai baru janlup for all mahasiswa
        registeredNim.add(nim); //PENTING! tambahkan nim ke daftar registeredNim
    }
    ...
    public void addGrade(String kodeMatkul, Double nilai) throws InvalidScoreException, DuplicateSubjectException {
        if (nilai < 0 || nilai > 100) { //cek nilai valid ga
            throw new InvalidScoreException(); //kl ga valid throw exception
        }
        if (this.nilai.containsKey(kodeMatkul)) { //cek kode matkul ud ada di daftar nilai
            throw new DuplicateSubjectException(kodeMatkul + " already in grades!"); //kl udah ada throw exception
        }
        
        String index; //buat variabel index untuk menyimpan indeks nilai
        ...
        this.nilai.put(kodeMatkul, index); //masukin kode matkul dan indeks ke daftar nilai pake put
    }
...
class NimAlreadyExistsException extends Exception {
    public String getMessage() { //bikin method getMessage
        // Implementasi custom message NimAlreadyExistsException
        // Return pesan multak berisi: "NIM is already registered!"
        return "NIM is already registered!"; // pesan kalo nim ud terdaftar
    }
}
...
class InvalidScoreException extends Exception {
    public String getMessage() { //bikin method getMessage
        ...
        return "Score cannot be below 0 or above 100!"; // pesan kalo nilai ga valid
    }
}

class DuplicateSubjectException extends Exception {
    public DuplicateSubjectException(String message) { //bikin constructor dg parameter message
        // Implementasi DuplicateSubjectException
        super(message); // manggil constructor dari superclass Exception dengan message
    }
}
