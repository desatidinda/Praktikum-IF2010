...
/**
    ...
    public static void validatePacket(List<Node> nodes, Map<String, Set<Integer>> transmissionLog, Packet packet) throws NetworkException {
        if (packet.to.equals(packet.from)) {
            throw new LoopbackException(packet.from);
        }
        Node sourceNode = findNode(nodes, packet.from);
        Node destinationNode = findNode(nodes, packet.to);
        if (sourceNode == null || destinationNode == null) {
            throw new NetworkException();
        }
        if (packet.timestamp <= destinationNode.lastReceived) {
            throw new TimeDesyncException(packet.to, destinationNode.lastReceived, packet.from, packet.timestamp);
        }
        if (transmissionLog.containsKey(packet.to) && transmissionLog.get(packet.to).contains(packet.timestamp)) {
            throw new CollisionException(packet.to, packet.timestamp);
        }

        destinationNode.lastReceived = packet.timestamp;
...
class LoopbackException extends NetworkException {
    public LoopbackException(String nodeId) {
        super("Loopback: Node " + nodeId + " tidak boleh mengirim ke dirinya sendiri.");
    }
}
...
class TimeDesyncException extends NetworkException {
    public TimeDesyncException(String nodeId, int lastReceivedTimestamp, String fromNode, int newTimestamp) {
        super("Time desync: Node " + nodeId + " sudah menerima hingga waktu " + lastReceivedTimestamp + 
              ", tetapi kemudian menerima dari " + fromNode + " dengan waktu " + newTimestamp);
    }
}
...
class CollisionException extends NetworkException {
    public CollisionException(String nodeId, int timestamp) {
        super("Collision: Ada lebih dari satu paket ke node " + nodeId + " pada waktu " + timestamp);
    }
}
