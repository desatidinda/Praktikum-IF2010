...
public class PetriDishManager {
     ...
     * Hint: Gunakan method reproduce() dari interface BacteriaBehaviour.
     * @param days jumlah hari inkubasi
     */
    public void incubate(int days) {
        for (BacteriaBehaviour b : colony) {
            if (b != null) {
                if (cultureCondition.equals("Rich")) {
                    if (kanamycinPresence && b.hasKanR() == false) {
                        b.reproduce(days, 1.0 * 0.7);
                    }
                    else {
                        b.reproduce(days, 1.0);
                    }
                }
                else if (cultureCondition.equals("Limited")) {
                    if (kanamycinPresence && b.hasKanR() == false) {
                        b.reproduce(days, 0.8 * 0.7);
                    }
                    else {
                        b.reproduce(days, 0.8);
                    }
                }
                else if (cultureCondition.equals("Toxic")) {
                    if (kanamycinPresence && b.hasKanR() == false) {
                        b.reproduce(days, 0.6 * 0.7);
                    }
                    else {
                        b.reproduce(days, 0.6);
                    }
                }
            }
        }
